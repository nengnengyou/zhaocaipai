		...
		<swiper :disable-touch="disableTouch" @change="change" :autoplay="autoplay && playState == 'play'" :vertical="vertical" circular :interval="duration" class="u-swiper">
			<swiper-item v-for="(item, index) in list" :key="index" class="u-swiper-item">
				...
				<view class="flex justify-between" style="width: 100%;">
					<text decode="true" class="u-news-item u-line-1" :style="[textStyle]" style="padding-right: 20rpx;" @tap="click(index)" :class="['u-type-' + type]">
						{{ item.mobile }}
					</text>
					<text decode="true" class="u-news-item u-line-1" :style="[textStyle]" style="width: 68%;text-align: left;" @tap="click(index)" :class="['u-type-' + type]">
						{{ item.auction_car_title }}
					</text>
					<text decode="true" class="u-news-item u-line-1" :style="[textStyle]" @tap="click(index)" :class="['u-type-' + type]">中标</text>
				</view>
			</swiper-item>
		</swiper>
		<view class="u-icon-wrap">
