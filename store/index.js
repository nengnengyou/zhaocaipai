...
// import jpushIM from '../common/im/im.js'
...
const store = new Vuex.Store({
	...
	mutations: {
		...
		modifyUpdateTips(state, type) {
			state.updateTips = type
		},
		...
		imLogin(state, val) {
			...
			state.imHasLogin = true;
			state.loginProvider = val; // 用户名
		},
		imLogout(state) {
			state.imHasLogin = false
			state.loginProvider = ""
			state.openid = null
					...
					switch (params.msgType) {
						...
						case "image":
							...
							jpushIM.sendImageMessage(params, (res) => {
								if (res.errorCode == 0) {
									console.log("JMessagePlugin sendImageMessage:" + JSON
										.stringify(res));
									resolve(res.data);
								} else {
									reject(res.errorMsg);
						...
						case "voice":
							...
							jpushIM.sendVoiceMessage(params, (res) => {
								if (res.errorCode == 0) {
									console.log("JMessagePlugin sendVoiceMessage:" + JSON
										.stringify(res));
									resolve(res.data);
								} else {
									reject(res.errorMsg);
						...
						case "location":
							...
							jpushIM.sendLocationMessage(params, (res) => {
								if (res.errorCode == 0) {
									console.log("JMessagePlugin sendLocationMessage:" + JSON
										.stringify(res));
									resolve(res.data);
								} else {
									reject(res.errorMsg);
